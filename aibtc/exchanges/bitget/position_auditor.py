# exchanges/bitget/position_auditor.py
"""
Bitget 持仓审计器

定期比对 Redis 中的持仓数据和 Bitget 交易所的实际持仓，修复不一致
"""
...
class AuditReport:
    ...
    def summary(self) -> str:
        lines = [
            f"[BITGET AUDIT REPORT] ts={self.timestamp} duration={self.duration_ms}ms",
...
class BitgetPositionAuditor:
    ...
    # Bitget 认证相关错误码
    AUTH_ERROR_CODES = {'40001', '40002', '40003', '40004', '40005', '40006', '40007', '40008', '40009', '40010'}

    def __init__(
        self,
        exchange_client,  # BitgetExchange instance
        redis_conn,
        uid: str,
        *,
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            ...
            name=f"bitget-position-auditor-{self.uid}",
            daemon=True,
        )
        self._thread.start()
        logger.info(f"[{self.uid}][bitget] PositionAuditor 已启动")
    ...
    def stop(self) -> None:
        ...
        logger.info(f"[{self.uid}][bitget] PositionAuditor 已停止")
    ...
    def _is_auth_error(self, error_msg: str) -> bool:
        ...
        for code in self.AUTH_ERROR_CODES:
            if code in error_msg:
                return True
        auth_keywords = ['Invalid apiKey', 'signature', 'Signature', 'API key', 'api key']
    ...
    def _run_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if self._auth_failed:
                logger.warning(f"[{self.uid}][bitget] PositionAuditor 因认证失败已停止")
                break
            ...
            try:
                ...
                if report.error and self._is_auth_error(report.error):
                    ...
                    logger.error(f"[{self.uid}][bitget] PositionAuditor 检测到认证错误，停止审计: {report.error}")
                ...
                logger.exception("Bitget 审计循环错误")
    ...
    def _fetch_exchange_positions(self) -> Dict[str, dict]:
        ...
        try:
            ...
            logger.error(f"[{self.uid}][bitget] 获取持仓失败: {e}")
            logger.exception("Bitget 获取持仓失败")
    ...
    def _fix_issues(self, issues: List[AuditIssue], exchange_pos: Dict[str, dict]) -> int:
        ...
        for issue in issues:
            ...
            try:
                ...
                logger.exception("Bitget 修复问题失败")
    ...
    def _close_ghost_cycle_and_cleanup(self, field: str, close_time_ms: int) -> bool:
        ...
        logger.info(f"[{self.uid}][bitget] Ghost cycle closed: {field} cycleId={cycle_id} netPnl={cyc['netPnl']}")
